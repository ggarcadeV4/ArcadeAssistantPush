...
TTS (Text-to-Speech) — Phase 3:
  check_tts_availability() → (available: bool, reason: str | None)
  speak_stream(...)        → async generator of MP3 bytes
  speak(...)               → full MP3 bytes
  get_voices(...)          → list of available ElevenLabs voices
"""
...
# TTS imports (graceful — httpx may not be installed yet)
try:
    from .tts import (
        ...
        speak,
        get_voices,
        test_connection as test_tts_connection,
    )
    _tts_available = True
except ImportError:
