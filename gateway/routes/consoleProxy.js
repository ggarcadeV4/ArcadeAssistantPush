...
// Proxy all console requests (including TeknoParrot) to FastAPI
router.use('/', enforceScopeHeader, async (req, res) => {
  try {
    ...
    // Remove /api/console prefix and forward the rest to /api/local/console
    const pathWithQuery = req.originalUrl.replace('/api/console', '');
    const targetUrl = `${fastapiUrl}/api/local/console${pathWithQuery}`;
    ...
    console.error('Console proxy error:', err);

    if (err.code === 'ECONNREFUSED') {
      res.status(503).json({
        error: 'FastAPI unavailable',
        message: 'Console service is not running'
      });
    } else {
      res.status(500).json({
        error: 'Proxy error',
