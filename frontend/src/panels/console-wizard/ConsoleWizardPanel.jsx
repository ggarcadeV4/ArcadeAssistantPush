...
export default function ConsoleWizardPanel() {
  ...
  useEffect(() => {
    ...
    const checkChuckMapping = async () => {
      try {
        const data = await fetchJSON('/api/local/controller/mapping');
        if (cancelled) return;

        const mappingMissing = Array.isArray(data)
          ? data.length === 0
          : (data && typeof data === 'object'
            ? Object.keys(data).length === 0
            : !data);

        setChuckMappingMissing(mappingMissing);
      } catch (err) {
        if (cancelled) return;
        setChuckMappingMissing(err?.status === 404);
      }
    };
      ...
      <div className="console-wizard-panel" style={{ flex: 1, minWidth: 0 }}>
        {chuckMappingMissing ? (
          <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div className="panel-banner warning" style={{ maxWidth: 560, justifyContent: 'center', textAlign: 'center' }}>
              <div>
                <strong>DEPENDENCY MISSING</strong>
                <span>You must run Controller Chuck first to define your physical cabinet before mapping handhelds.</span>
              </div>
            </div>
          </div>
        ) : (
