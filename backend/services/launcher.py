...
class GameLauncher:
        ...
        try:
            ...
            if result.get("success"):
                return LaunchResponse(
                    ...
                    pid=result.get("pid"),  # Phase 0F: propagate PID for lifecycle tracking
                )
            logger.warning(
    ...
    @staticmethod
    def _aa_drive_root() -> Path:
        """Resolve the cabinet drive root with AA_DRIVE_ROOT override support."""
        return Path(os.getenv("AA_DRIVE_ROOT") or str(LaunchBoxPaths.AA_DRIVE_ROOT))

    @staticmethod
    def _canonical_platform_name(platform: str) -> str:
        """Map known LaunchBox aliases onto the platform names the launcher expects."""
        platform_key = normalize_key(platform or "")
        aliases = {
        ...
        return aliases.get(platform_key, platform)

    @staticmethod
    def _clone_game_with_platform(game: Game, platform_name: str) -> Game:
        """Create a shallow game copy with a canonicalized platform name."""
        if platform_name == getattr(game, "platform", None):
            return game
        try:
            return game.model_copy(update={"platform": platform_name})
        except Exception:
            payload = game.dict() if hasattr(game, "dict") else dict(game)
            payload["platform"] = platform_name
            return Game(**payload)

    @staticmethod
    def _resolve_launchbox_path(path_value: str) -> Path:
        """Resolve LaunchBox-relative paths against the current cabinet root."""
        path_str = str(path_value or "").replace("\\", "/")
        path_obj = Path(path_str)
        if path_str.startswith(".."):
            return (LaunchBoxPaths.LAUNCHBOX_ROOT / path_str).resolve()
    ...
    def _extract_ahk_command_field(line: str) -> Optional[str]:
        """Extract the AHK Run/RunWait command field without splitting quoted commas."""
    ...
    @classmethod
    def _extract_singe2_command(cls, ahk_path: Path) -> Tuple[Path, List[str], Path]:
        """Parse the first non-Sinden Run line from a SINGE2 AHK wrapper."""
        ...
        for raw_line in content.splitlines():
            ...
            if not (lower.startswith("run,") or lower.startswith("runwait,")):
                continue
            if "lightgun.exe" in lower:
                continue

            command_text = cls._extract_ahk_command_field(line)
            if not command_text:
                continue
            ...
            run_target = tokens[0].strip().strip('"')
            args = [token.strip() for token in tokens[1:]]
            exe_path = Path(run_target.replace("\\", "/"))
            if not exe_path.is_absolute():
                exe_path = (ahk_path.parent / exe_path).resolve()
            if not exe_path.exists() and not exe_path.suffix:
                exe_candidate = exe_path.with_suffix(".exe")
                if exe_candidate.exists():
                    exe_path = exe_candidate
            if exe_path.exists():
                return exe_path, args, ahk_path.parent

        raise ValueError(f"No executable Run command found in SINGE2 script: {ahk_path}")
    ...
    def _launch_via_detected_emulator(self, game: Game) -> Dict[str, Any]:
        ...
        launch_game = self._clone_game_with_platform(
            game, self._canonical_platform_name(getattr(game, "platform", "") or "")
        )
        if launch_game.platform != game.platform:
            logger.info(
                "[DetectedEmu] Platform alias resolved: '%s' -> '%s'",
                game.platform,
                launch_game.platform,
            )

        result = self.emulator_config.get_emulator_for_platform(launch_game.platform)

        if not result:
            raise ValueError(f"No emulator configured for platform: {launch_game.platform}")
        ...
        if is_pcsx2:
            ...
            rom_str = self._get_rom_path(launch_game)
            src = Path(rom_str)
        ...
        command = self._build_emulator_command(launch_game, emulator, mapping)
    ...
    def _launch_direct(self, game: Game, profile_hint: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Gun Build routing: detect raw gun platform names before normalize_key() strips "gun games"
            raw_platform = (getattr(game, "platform", "") or "").strip().lower()
            is_gun_platform = "gun" in raw_platform and "games" in raw_platform
            drive_root = self._aa_drive_root()
            launch_game = self._clone_game_with_platform(
                game, self._canonical_platform_name(getattr(game, "platform", "") or "")
            )
            if launch_game.platform != game.platform:
                logger.info(
                    "[DIRECT] Platform alias resolved: '%s' -> '%s'",
                    game.platform,
                    launch_game.platform,
                )
            gun_exe_override: Optional[Path] = None
            gun_core_override: Optional[str] = None
            GUN_PLATFORM_MAP = {
                'nes gun games':            {'emulator': 'retroarch_gun',  'core': 'mesen'},
                'master system gun games':  {'emulator': 'retroarch_gun',  'core': 'genesis_plus_gx'},
                'super nintendo gun games': {'emulator': 'snes9x_gun',     'core': None},
                'snes gun games':            {'emulator': 'snes9x_gun',     'core': None},
                'wii gun games':            {'emulator': 'dolphin_gun',    'core': None},
                'playstation 2 gun games':  {'emulator': 'pcsx2_gun',      'core': None},
                'ps2 gun games':            {'emulator': 'pcsx2_gun',      'core': None},
                'teknoparrot gun games':    {'emulator': 'teknoparrot_gun','core': None},
                'pc gun games':             {'emulator': 'retroarch_gun',  'core': None},
                'naomi gun games':          {'emulator': 'retroarch_gun',  'core': None},
                'atomiswave gun games':     {'emulator': 'retroarch_gun',  'core': None},
                'dreamcast gun games':      {'emulator': 'retroarch_gun',  'core': None},
                'ps3 gun games':            {'emulator': 'rpcs3_gun',      'core': None},
                'saturn gun games':         {'emulator': 'retroarch_gun',  'core': None},
                'model 2 gun games':        {'emulator': 'model2_gun',     'core': None},
                'model 3 gun games':        {'emulator': 'supermodel_gun', 'core': None},
                'pcsx2 gun games':          {'emulator': 'pcsx2_gun',      'core': None},
            }
            if is_gun_platform:
                gun_cfg = GUN_PLATFORM_MAP.get(raw_platform)
                if gun_cfg:
                    gun_exe_override = getattr(EmulatorPaths, str(gun_cfg["emulator"]))()
                    gun_core_override = gun_cfg.get("core")
                    logger.info("[Launcher] Gun platform detected: %s -> %s", raw_platform, gun_exe_override)
                else:
                    gun_exe_override = EmulatorPaths.retroarch_gun()
                    logger.warning("[Launcher] Gun platform fallback: %s -> %s", raw_platform, gun_exe_override)
            ...
            daphne_uses_mame = (
                launch_game.platform == "Daphne"
                and app_path_str.endswith((".zip", ".7z"))
            )
            platform_key = normalize_key(getattr(launch_game, "platform", "") or "")
            if platform_key in {"arcade", "arcade mame", "mame"} or daphne_uses_mame:
                ...
                if trap_result["success"]:
                    ...
                    if trap_result.get("pid"):
                        ...
                        logger.info(f"[DIRECT] MAME PID={trap_result['pid']} available for lifecycle tracking")
                    return result
                else:
            ...
            if platform_key == "nintendo ds":
                rom_path = self._resolve_launchbox_path(self._get_rom_path(launch_game))
                if not rom_path.exists():
                    raise FileNotFoundError(f"Nintendo DS ROM not found: {rom_path}")

                ds_candidates = [
                    (drive_root / "Emulators" / "melonDS" / "melonDS.exe", [str(rom_path)]),
                    (drive_root / "Emulators" / "DeSmuME" / "DeSmuME.exe", [str(rom_path)]),
                    (
                        drive_root / "Emulators" / "RetroArch Gamepad" / "retroarch.exe",
                        [
                            "-L",
                            str(drive_root / "Emulators" / "RetroArch Gamepad" / "cores" / "desmume_libretro.dll"),
                            "-f",
                            str(rom_path),
                        ],
                    ),
                    (
                        drive_root / "Emulators" / "RetroArch" / "retroarch.exe",
                        [
                            "-L",
                            str(drive_root / "Emulators" / "RetroArch" / "cores" / "desmume_libretro.dll"),
                            "-f",
                            str(rom_path),
                        ],
                    ),
                ]
                for ds_exe, ds_args in ds_candidates:
                    if not ds_exe.exists():
                        continue
                    if "retroarch" in ds_exe.name.lower() and not Path(ds_args[1]).exists():
                        continue
                    if "retroarch" in ds_exe.name.lower():
                        logger.warning(
                            "[DIRECT] Nintendo DS standalone emulator not installed; using RetroArch DeSmuME core fallback: %s",
                            ds_exe,
                        )
                    command = [str(ds_exe), *ds_args]
                    logger.info(
                        "[DIRECT] Nintendo DS launch: exe=%s, rom=%s, cwd=%s",
                        ds_exe,
                        rom_path,
                        ds_exe.parent,
                    )
                    win_command = _convert_wsl_paths_for_windows(command)
                    wsl_cwd = str(ds_exe.parent)
                    if dry_run_enabled():
                        logger.info(f"[DIRECT] Nintendo DS DRY-RUN: {' '.join(win_command)}")
                        return {"success": True, "command": " ".join(win_command), "message": "dry-run"}
                    trap_result = self._launch_with_stderr_trap(win_command, cwd=wsl_cwd)
                    if trap_result["success"]:
                        result = {"success": True, "command": " ".join(win_command)}
                ...
                raise FileNotFoundError(
                    "Nintendo DS emulator not found in AA_DRIVE_ROOT Emulators tree "
                    "(checked melonDS, DeSmuME, and RetroArch DeSmuME core fallbacks)"
                )

            # Daphne/Hypseus laserdisc direct launch
            if platform_key == "daphne" and not daphne_uses_mame:
                exe = drive_root / "Emulators" / "Hypseus" / "Hypseus Singe" / "hypseus.exe"
                homedir = drive_root / "Roms" / "DAPHNE"

                daphne_game_map = {
                    "astron belt": ("astron", drive_root / "Roms" / "DAPHNE" / "framefile" / "astron.txt"),
                    "badlands": ("badlands", drive_root / "Roms" / "DAPHNE" / "framefile" / "badlands.txt"),
                    "bega's battle": ("bega", drive_root / "Roms" / "DAPHNE" / "framefile" / "bega.txt"),
                    "cobra command": ("cobraab", drive_root / "Roms" / "DAPHNE" / "framefile" / "cobraab.txt"),
                    "dragons lair": ("lair", drive_root / "Roms" / "DAPHNE" / "vldp" / "lair" / "lair.txt"),
                    "dragon's lair hd": ("lair", drive_root / "Roms" / "DAPHNE" / "vldp" / "lair" / "lair.txt"),
                    "dragon's lair ii hd": ("lair2", drive_root / "Roms" / "DAPHNE" / "vldp" / "lair2" / "dl2-framefile.txt"),
                    "galaxy ranger": ("galaxy", drive_root / "Roms" / "DAPHNE" / "framefile" / "galaxy.txt"),
                    "gp world": ("gpworld", drive_root / "Roms" / "DAPHNE" / "framefile" / "gpworld.txt"),
                    "inter stellar": ("interstellar", drive_root / "Roms" / "DAPHNE" / "framefile" / "interstellar.txt"),
                    "mach 3": ("mach3", drive_root / "Roms" / "DAPHNE" / "framefile" / "mach3.txt"),
                    "road blaster": ("roadblaster", drive_root / "Roms" / "DAPHNE" / "framefile" / "roadblaster.txt"),
                    "space ace hd": ("ace", drive_root / "Roms" / "DAPHNE" / "vldp" / "ace" / "ace.txt"),
                    "super don quix-ote": ("sdq", drive_root / "Roms" / "DAPHNE" / "framefile" / "sdq.txt"),
                    "thayer's quest": ("tq", drive_root / "Roms" / "DAPHNE" / "framefile" / "tq.txt"),
                    "us vs them": ("uvt", drive_root / "Roms" / "DAPHNE" / "framefile" / "uvt.txt"),
                }
                ...
                if ahk_stem not in daphne_game_map:
                    ...
                    logger.warning(
                        "WARNING: Daphne game '%s' not in DAPHNE_GAME_MAP - attempting best-effort launch",
                        ahk_stem,
                    )
                    if not framefile.exists():
                        logger.warning(
                            "WARNING: Daphne best-effort framefile missing: %s",
                            framefile,
                        )
                        return self._launch_via_launchbox(game)
                    game_name = ahk_stem
                else:
                ...
                if trap_result["success"]:
                    ...
                    if trap_result.get("pid"):
                        result["pid"] = trap_result["pid"]
                        logger.info(f"[DIRECT] Hypseus PID={trap_result['pid']} available for lifecycle tracking")
                    return result
            ...
            # SINGE2 direct launch support for builds that expose a dedicated SINGE2 platform.
            # Working-pattern titles confirmed by wrapper review:
            # - Cabinet-side titles: Altered Carbon, Asterix, Cliffhanger
            # - Gun Build titles: 38 Ambush Alley, Blue Thunder, Crime Patrol, Drug Wars,
            #   Hydrosub 2021, Mad Dog McCree, Mad Dog McCree 2 The Lost Gold, Platoon,
            #   Pops Ghostly, Sonic Fury, Space Pirates, The Last Bounty Hunter, Who Shot Johnny Rock
            # Untested in this session: remaining SINGE2 titles and the Marbella Vice WinUAE variant.
            if platform_key == "singe2":
                app_path = str(getattr(launch_game, "application_path", "") or "").strip()
                if not app_path:
                    raise ValueError(f"No ApplicationPath configured for SINGE2 game: {launch_game.title}")
                ahk_path = self._resolve_launchbox_path(app_path)
                ...
                command = [str(singe_exe), *singe_args]
                logger.info(
                    "[DIRECT] SINGE2 launch: exe=%s, script=%s, cwd=%s",
                    singe_exe,
                    ahk_path,
                    singe_cwd,
                )
            ...
            for adapter in ADAPTERS:
                ...
                try:
                    can_handle = adapter.can_handle(launch_game, manifest)
                    if not can_handle:
                        continue
                ...
                logger.info(f"[DIRECT] Trying adapter: {adapter_name} for platform={launch_game.platform}")
                try:
                    cfg = adapter.resolve(launch_game, manifest)
                except Exception as e:
                ...
                if exe and gun_exe_override:
                    ...
                    if _app_path.lower().endswith('.ahk') and is_gun_platform:
                        ...
                        if _ahk_exe.exists():
                            exe = str(_ahk_exe)
                            args = [_app_path]
                            cwd = str(_ahk_exe.parent)
                            logger.info("[Launcher] AHK override applied for %s: %s", raw_platform, _app_path)
                        else:
                            logger.error("[Launcher] AutoHotkey not found for gun AHK game: %s", _app_path)
                            exe = str(gun_exe_override)
                            cwd = str(gun_exe_override.parent)
                    else:
                        exe = str(gun_exe_override)
                        cwd = str(gun_exe_override.parent)
                    if gun_core_override and "retroarch" in adapter_name:
                        cores = ((manifest.get("retroarch") or {}).get("cores") or {}) if isinstance(manifest, dict) else {}
                        core_rel = cores.get(gun_core_override)
                        if core_rel:
                            core_path = Path(core_rel)
                            if not core_path.is_absolute():
                                candidate = gun_exe_override.parent / core_rel
                                if not candidate.exists():
                                    candidate = gun_exe_override.parent / "cores" / core_rel
                                core_path = candidate
                            if len(args) >= 2 and args[0] == "-L":
                                args[1] = str(core_path)
                            else:
                                args = ["-L", str(core_path), *args]
                        else:
                            logger.warning("[Launcher] Gun core mapping missing: %s for %s", gun_core_override, raw_platform)
            ...
            # ----- Gun platform direct-launch fallback -----
            # If gun_exe_override was set but no adapter resolved an exe,
            # build the command directly from the gun emulator + ROM path.
            # Also handles .ahk wrapper scripts via AutoHotkey.
            if gun_exe_override:
                ...
                if app_path.lower().endswith('.ahk') and is_gun_platform:
                    ...
                    if ahk_exe.exists():
                        gun_exe = str(ahk_exe)
                        gun_args = [app_path]
                        gun_cwd = str(ahk_exe.parent)
                        logger.info("[Launcher] Gun AHK wrapper: %s -> %s %s",
                                    raw_platform, gun_exe, app_path)
                    else:
                        logger.error("[Launcher] AutoHotkey not found for gun AHK game: %s", app_path)
                        raise FileNotFoundError(f"AutoHotkey not found for gun AHK: {app_path}")
                else:
                    # --- Standard gun emulator launch ---
                    rom_path = self._resolve_rom_path(game)
                    gun_exe = str(gun_exe_override)
                    gun_cwd = str(gun_exe_override.parent)
                    gun_args: list = []

                    # NES Gun Games: hard-wire nestopia core (mesen_libretro.dll not on disk)
                    if raw_platform == 'nes gun games' and "retroarch" in gun_exe.lower():
                        nes_core = gun_exe_override.parent / "cores" / "nestopia_libretro.dll"
                        if nes_core.exists():
                            gun_args = ["-L", str(nes_core), str(rom_path)]
                            logger.info("[Launcher] NES Gun: using nestopia core at %s", nes_core)
                        else:
                            # Try fceumm as secondary NES core
                            fceumm = gun_exe_override.parent / "cores" / "fceumm_libretro.dll"
                            if fceumm.exists():
                                gun_args = ["-L", str(fceumm), str(rom_path)]
                                logger.info("[Launcher] NES Gun: using fceumm core at %s", fceumm)
                            else:
                                logger.error("[Launcher] No NES core found for Gun Build RetroArch")
                                gun_args = [str(rom_path)]
                    # Other RetroArch-based guns: resolve core from cores/ directory
                    elif "retroarch" in gun_exe.lower():
                        if gun_core_override:
                            cores_dir = gun_exe_override.parent / "cores"
                            for suffix in ["_libretro.dll", ".dll"]:
                                candidate = cores_dir / f"{gun_core_override}{suffix}"
                                if candidate.exists():
                                    gun_args = ["-L", str(candidate), str(rom_path)]
                                    break
                            if not gun_args:
                                gun_args = ["-L", gun_core_override, str(rom_path)]
                        else:
                            gun_args = [str(rom_path)]
                    else:
                        # Non-RetroArch gun emulators (PCSX2, Dolphin, etc.)
                        gun_args = [str(rom_path)]

                logger.info("[DIRECT] Gun platform direct-launch: exe=%s, args=%s, cwd=%s", gun_exe, gun_args, gun_cwd)
                full_cmd = [gun_exe, *[str(a) for a in gun_args]]
                win_cmd = _convert_wsl_paths_for_windows(full_cmd)
                if dry_run_enabled():
                    return {"success": True, "command": " ".join(win_cmd), "message": "dry-run (gun)"}
                trap_result = self._launch_with_stderr_trap(win_cmd, cwd=gun_cwd)
                if trap_result["success"]:
                    logger.info("[DIRECT] Gun platform launch SUCCESS: %s", " ".join(win_cmd))
                    result = {"success": True, "command": " ".join(win_cmd)}
                    if trap_result.get("pid"):
                        result["pid"] = trap_result["pid"]
                    return result
                else:
                    logger.warning("[DIRECT] Gun platform launch FAILED (code %s)", trap_result.get('return_code'))
                    return {
                        "success": False,
                        "command": " ".join(win_cmd),
                        "stderr_trap": trap_result,
                    }
    ...
    def _build_mame_command(rom_path: Path) -> List[str]:
        ...
        try:
            cfg = GameLauncher._load_launchers_config()
            mame_cfg = (cfg or {}).get("mame", {})
            # extra flags
                ...
                if fp.exists():
                    ...
                    if (GameLauncher._launchers_config_cache is None) or (GameLauncher._launchers_config_mtime != mtime):
                        with open(fp, 'r', encoding='utf-8') as f:
                            GameLauncher._launchers_config_cache = json.load(f)
