...
Routes Blinky AI calls through SecureAIClient using panel=blinky.
"""
...
_CANONICAL_DRIVE_ROOT = get_drive_root(context="blinky_chat_service")
...
def _prompt_candidates() -> List[Path]:
    ...
    return [
        project_root / "prompts" / "blinky.prompt",
        _CANONICAL_DRIVE_ROOT / "prompts" / "blinky.prompt",
    ]
...
def _load_prompt_variants() -> Dict[str, str]:
    ...
    if prompt_text is None:
        raise FileNotFoundError(
            f"panel=blinky prompt file not found; tried: {_prompt_candidates()}"
        )
...
def chat_with_blinky(
    *,
    ...
    logger.info(
        "panel=blinky service_start cabinet_id=%s requested_diagnosis_mode=%s history_turns=%s",
        resolved_cabinet_id,
        is_diagnosis_mode,
        len(history or []),
    )
    ...
    if not reply:
        raise ValueError("panel=blinky SecureAIClient returned an empty response")

    provider = result.get("provider", "unknown")
    model = result.get("model", "unknown")
    logger.info(
        "panel=blinky service_complete cabinet_id=%s provider=%s model=%s",
        resolved_cabinet_id,
        provider,
        model,
    )
