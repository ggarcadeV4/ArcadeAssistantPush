...
Routes Wiz AI calls through SecureAIClient using panel=wiz.
"""
...
_CANONICAL_DRIVE_ROOT = get_drive_root(context="wiz_service")
...
def _prompt_candidates() -> List[Path]:
    ...
    return [
        project_root / "prompts" / "controller_wizard.prompt",
        _CANONICAL_DRIVE_ROOT / "prompts" / "controller_wizard.prompt",
    ]
...
def _knowledge_candidates() -> List[Path]:
    project_root = Path(__file__).resolve().parents[2]
    return [
        project_root / "prompts" / "wiz_knowledge.md",
        _CANONICAL_DRIVE_ROOT / "prompts" / "wiz_knowledge.md",
    ]
...
def _load_prompt_variants() -> Dict[str, str]:
    ...
    if prompt_text is None:
        raise FileNotFoundError(
            f"panel=wiz prompt file not found; tried: {_prompt_candidates()}"
        )
...
def chat_with_wiz(
    *,
    ...
    logger.info(
        "panel=wiz service_start cabinet_id=%s requested_diagnosis_mode=%s history_turns=%s",
        resolved_cabinet_id,
        is_diagnosis_mode,
        len(history or []),
    )
    ...
    if not reply:
        raise ValueError("panel=wiz SecureAIClient returned an empty response")

    provider = result.get("provider", "unknown")
    model = result.get("model", "unknown")
    logger.info(
        "panel=wiz service_complete cabinet_id=%s provider=%s model=%s",
        resolved_cabinet_id,
        provider,
        model,
    )
