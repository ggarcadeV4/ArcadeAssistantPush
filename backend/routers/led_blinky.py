...
class LEDMapping(BaseModel):
    ...
    @classmethod
    def prefer_buttons_field(cls, values: Any) -> Any:
    ...
    def validate_scope_and_buttons(self) -> "LEDMapping":
        scope = str(self.scope or "game").strip().lower()
        ...
        if scope == "game" and not (self.game or self.profile_name):
...
async def log_led_change(request: Request, drive_root: Path, action: str, details: Dict[str, Any], backup_path: Optional[Path] = None):
