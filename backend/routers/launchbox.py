...
async def launch_game(
    ...
    try:
        ...
        if forced_method in {"plugin", "detected_emulator", "direct", "launchbox"}:
            ...
            try:
                ...
                pid = getattr(result, "pid", None) or _best_effort_find_pid(exe_path)  # Phase0F: prefer direct PID
                if pid:
                ...
                logger.error(f"[Phase0F] PID tracking FAILED in policy_primary path: {e}", exc_info=True)
            return result
    except Exception:
        ...
        try:
            ...
            if direct_result and getattr(direct_result, 'success', False):
                ...
                try:
                    ...
                    pid = getattr(direct_result, "pid", None) or _best_effort_find_pid(exe_path)  # Phase0F: prefer direct PID
                    if pid:
                        from backend.services.game_lifecycle import track_game_launch
    ...
    try:
        ...
        # Phase 0F: PID tracking for plugin_first fallback path
        try:
            if getattr(result, "success", False):
                exe_path, emulator_name, _ = _parse_command_info(getattr(result, "command", None))
                pid = getattr(result, "pid", None) or _best_effort_find_pid(exe_path)
                if pid:
                    from backend.services.game_lifecycle import track_game_launch
                    track_game_launch(
                        game_id=game.id,
                        ...
                        rom_name=Path(game.rom_path).stem if getattr(game, "rom_path", None) else None,
                        source="launchbox_router",
                        launch_method="plugin_first_fallback",
                    )
        except Exception as e:
            pass
        return result
